...
void pvm_parse(const TraceEvent &tr,
  ...
  if (par_chk.second) {
        executions->push_back(new DBCreateNode(par->get_db_id(),
                                           tr.subjprocuuid,
                                           tr.pid,
                                           tr.exec));
  }
  if (tr.event == "audit:event:aue_execve:") {
      if (par->get_thin()) {
        ...
        par->set_thin(false);
        executions->push_back(new DBUpdateNode(par->get_db_id(),
                                           tr.pid,
                                           tr.cmdline));
      } else {
        ...
        executions->push_back(new DBCreateNode(next->get_db_id(),
                                           ...
                                           tr.pid,
                                           tr.cmdline));
        executions->push_back(new DBCreateRel(par->get_db_id(),
                                          next->get_db_id(),
                                          std::string("next")));
      }
      ...
      if (ch_chk.second) {
        executions->push_back(new DBCreateNode(ch->get_db_id(),
                                           tr.ret_objuuid1,
                                           tr.retval,
                                           par->get_cmdline()));
      } else {
        ch->set_cmdline(par->get_cmdline());
        executions->push_back(new DBUpdateNode(ch->get_db_id(),
                                           tr.retval,
                                           par->get_cmdline()));
      }
      executions->push_back(new DBCreateRel(par->get_db_id(),
